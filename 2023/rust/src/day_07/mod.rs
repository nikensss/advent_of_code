...
fn parse_line(input: &str) -> IResult<&str, Hand> {
    ...
    let cards = cards
        .chars()
        .map(|c| Card::new(&c.to_string()).unwrap())
        .collect();
...
mod tests {
    ...
    const TEST_INPUT: &str = include_str!("../day_07/test_input.txt");
    const COMPLETE_INPUT: &str = include_str!("../day_07/complete_input.txt");
